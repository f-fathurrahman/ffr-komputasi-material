...
from ch13.diis import DIIS
from ch16.krylov import solve_krylov
from ch16.rhf_grad import grad_hcore, grad_hf_energy, solve_mo1
...
def CCD_solve(coords, mf: pyscf.scf.hf.RHF, max_cycle=20, conv_tol=1e-5):
...
if __name__ == '__main__':
    ...
    mf.reset().run()
    e_ccd0 = CCD_solve(coords, mf)

    coords[0,0] -= delta
    mol.set_geom_(coords, unit='Bohr')
    mf.reset().run()
    e_ccd1 = CCD_solve(coords, mf)
    print((e_ccd0 - e_ccd1)/delta)
