...
from ch12_analytical_integrals_v5 import eri_OS_rys
...
print("get_eri_tensor: ", timing(eri_OS_rys.get_eri_tensor, gtos))

gtos = List(gto.to_jitCGTO() for gto in gtos)
print("get_eri_tensor_jit: ", timing(eri_OS_rys.get_eri_tensor_jit, gtos))

from pyscf import gto
mol = gto.M(atom=benzene, basis={'C': '6-31g*', 'H': '6-31g'})
print(f"nbas = {mol.nbas}, nao_cart = {mol.nao_cart()}")
t0 = time.perf_counter()
v = mol.intor('int2e_cart', aosym='s1')
t1 = time.perf_counter()
print("From pyscf int2e_cart: ", t1 - t0)
