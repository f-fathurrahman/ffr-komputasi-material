...
def iter_cart_xyz(n: int):
    '''Produces all (ix,iy,iz) subject to ix + iy + iz = n'''
    return [(i, j, n-i-j)
...
def num_functions(gtos):
    return gto_offsets(gtos)[-1]
...
class Molecule:
    ...
    def nuclear_charges(self):
        return [PERIODIC_TABLE[elem] for elem in self.elements]

    def nuclear_repulsion_energy(self):
        z = np.array(self.nuclear_charges)
        r = np.array(self.coordinates)
        d = np.linalg.norm(r[:,None,:] - r, axis=2)
        zz = z[:,None] * z
        tril = np.tril_indices(z.size, -1)
