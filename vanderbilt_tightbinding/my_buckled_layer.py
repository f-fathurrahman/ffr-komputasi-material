...
buckled_layer = TightBindingModel(2, 3, lat, orb)
...
buckled_layer.set_onsite([-delta,delta])
# set hoppings (one for each connected pair of orbitals)
# (amplitude, i, j, [lattice vector to cell containing j])
buckled_layer.set_hop(t, 1, 0, [0, 0, 0])
buckled_layer.set_hop(t, 1, 0, [1, 0, 0])
buckled_layer.set_hop(t, 1, 0, [0, 1, 0])
buckled_layer.set_hop(t, 1, 0, [1, 1, 0])

# print tight-binding model
buckled_layer.display()
...
(k_vec,k_dist,k_node)=buckled_layer.k_path(path,81)
# inputs:
#   path: see above
...
evals=buckled_layer.solve_all(k_vec)
...
# save as PDF
fig.tight_layout()
fig.savefig("IMG_buckled_layer.pdf")
