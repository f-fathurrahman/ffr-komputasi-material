function [lambda,phi,err_post,dof,vec_k,H,M,vext,HV,u,weight] = eigen_planewave_2D(V,L, Ec, Neig, MAX_Ec)
...
for ii = 1:dof
    for j = 1:dof
       ...
       % external potential H_{pq}=vext_fft(p-q)
        ...
        H(ii,j) = H(ii,j) + real(vext_fft(dk(1),dk(2)));  
        HV(ii,j) = real(vext_fft(dk(1),dk(2)));    
    end
end
%%
...
% evaluate the a posteriori error ||(V-V_n)\psi_n||
    err_post=zeros(Neig);
    phi_m = zeros(m, m, Neig);
    Vphi_m = zeros(m, m, Neig);
    u=zeros(m,m,Neig);
    for ll = 1:Neig
        ...
        u(:,:,ll)=ifftn(phi_m(:,:,ll))*(m^2)/(2*L);
        Vu=ifftn(Vphi_m(:,:,ll))*(m^2)/(2*L);
        deltaVphi = vext .* u(:,:,ll) - Vu;
        % Then transform the residual from real space back to k-space
        % to compute the H^{-1} norm
        res_fft = fftn(deltaVphi)*(2*L)/(m^2);
        res_weight = reshape( real(res_fft).^2 .* weight , m^2, 1);
