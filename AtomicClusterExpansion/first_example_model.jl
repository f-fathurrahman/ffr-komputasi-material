...
#
# Next we need to generate some training data to estimate the model parameters.
# Normally one would generate a training set using DFT data, stored as an `.xyz` file.
# Here, we create a random training set for simplicity. Please note that this is generally *not*
# a good strategy to generate data!
#
# * `gen_dat()` generates a single training configuration wrapped in an
#   `ACE1pack.AtomsData` structure. Each `d::AtomsData` contains the structure `d.atoms`,
# and energy value and a force vector to train against. 
...
#
# We specify a solver and then let `ACEfit.jl` to do all the work for us. More fine-grained
# control is possible; see the `ACEfit.jl` documentation.
# For sake of illustration we use a Bayesian Ridge Regression solver. This will
# automatically determine the regularisation for us. 
...
#
# At a minimum one should have a test set, check the errors on that test set,
# and confirm that they are similar as the training errors. 
...
# 
# The fitted model can be exported to a JSON or YAML file, or to a LAMMPs
# compatible `yace` file. We won't go through that in this tutoral.
# See `export2json` and `export2lammps` for further information. 
...
# Note that there are no vacancy structures in the training set, so this is a prediction
# out of sample. We have no guarantee of the accuracy of this prediction. In fact
# the prediction is quite far off: 
...
# To obtain accurate predictions on a vacancy structure, we must add it to the training set.
# This iterative model development process goes beyond the scope of this tutorial.
