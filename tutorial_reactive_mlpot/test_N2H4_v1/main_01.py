...
dataset = TensorDataset(qm_coord, atom_types_all, energy, qm_gradient)
train, val = random_split(dataset, [150, 50])
...
ene_pred, grad_pred = model(qm_coord, atom_types_all[0])

ene1 = energy.detach().numpy()
ene2 = ene_pred.detach().numpy()
rmse = np.sqrt( np.average( (ene1 - ene2)**2 ) )
print("RMSE energy = ", rmse)
# RMSE energy =  1.3540512, epoch

"""

import matplotlib.pyplot as plt
plt.clf();
min_e = min(ene1.min(), ene2.min())
max_e = max(ene1.max(), ene2.max())
plt.scatter(ene1, ene2)
plt.xlim(min_e, max_e)
plt.ylim(min_e, max_e)
plt.gca().set_aspect("equal", "box")
plt.show()
plt.clf(); plt.scatter(ene1, ene2); plt.show()
"""