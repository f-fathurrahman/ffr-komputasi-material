...
def prepare_data():
    ...
    qm_coord = np.array(np.load("../DATASET/N2H4_v1/coords.npy"), dtype="float32")

    atom_types = np.load("../DATASET/N2H4_v1/atomic_numbers.npy")
    elems = np.unique(atom_types).tolist()
    atom_types = np.array([[elems.index(i) for i in atom_types]])
    atom_types = atom_types.repeat(len(qm_coord), axis=0)

    energy = np.array(np.load("../DATASET/N2H4_v1/energies.npy"), dtype="float32")
    #energy = energy - energy.mean()

    qm_gradient = -np.array(np.load("../DATASET/N2H4_v1/forces.npy"), dtype="float32")
