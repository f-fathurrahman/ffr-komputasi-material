...
% this the "normal" SCF loop ?
function S = scf_loop(varargin)

fprintf('\n ----- ENTER the normal scf_loop ----- \n');
...
while count_SCF <= S.MAXIT_SCF
    ...
    if S.MixingVariable == 0
        [S.Etotal,S.Eband,S.Exc,S.Exc_dc,S.Eelec_dc,S.Eent] = my_evaluateTotalEnergy(S);
    end
    ...
    if S.spin_typ == 0
        fprintf('count_SCF = %-6d\n', count_SCF);
        fprintf('Etot per atom = %18.10E\n', S.Etotal/S.n_atm);
        fprintf('err = %.3E\n', err);
        fprintf('scf_runtime = %.3f\n', scf_runtime);
    elseif S.spin_typ == 1
        fprintf('count_SCF = %-6d\n', count_SCF);
        fprintf('Etot per atom = %18.10E\n', S.Etotal/S.n_atm);
        fprintf('net magn = %11.4E\n', S.netM);
        fprintf('err = %.3E\n', err);
        fprintf('scf_runtime = %.3f\n', scf_runtime);
    elseif S.spin_typ == 2
        fprintf('count_SCF = %-6d\n', count_SCF);
        fprintf('Etot per atom = %18.10E\n', S.Etotal/S.n_atm);
        fprintf('net magn = %11.4E, %11.4E, %11.4E, %11.4E\n', S.netM);
        fprintf('err = %.3E\n', err);
        fprintf('scf_runtime = %.3f\n', scf_runtime);
    end
...
fprintf('Total number of SCF: %-6d\n', count_SCF-1);

fprintf('\n ----- EXIT the normal scf_loop ----- \n');

end % end of scf_loop (inner function)
...
Veff = Veff(:);
end % end of function shifting_Veff
