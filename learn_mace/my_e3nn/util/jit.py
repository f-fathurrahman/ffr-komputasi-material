...
def compile_mode(mode: str):
    ...
    def decorator(obj):
        if not (inspect.isclass(obj) and issubclass(obj, torch.nn.Module)):
            raise TypeError("@my_e3nn.util.jit.compile_mode can only decorate classes derived from torch.nn.Module")
...
def get_tracing_inputs(
    ...
    Otherwise, attempts to infer the input signature of the module using ``my_e3nn.util._argtools._get_io_irreps``.
