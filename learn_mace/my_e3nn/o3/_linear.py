...
class Linear(CodeGenMixin, torch.nn.Module):
    ...
    Notes
    -----
        `my_e3nn.o3.Linear` objects created with different partitionings of the same irreps, such as ``Linear("10x0e", "0e")`` and ``Linear("3x0e + 7x0e", "0e")``, are *not* equivalent: the second module has more instructions, which affects normalization. In a rough sense:
    ...
    Parameters
    ----------
    irreps_in : `my_e3nn.o3.Irreps`
        representation of the input

    irreps_out : `my_e3nn.o3.Irreps`
        representation of the output

    internal_weights : bool
        whether the `my_e3nn.o3.Linear` should store its own weights. Defaults to ``True`` unless ``shared_weights`` is explicitly set to ``False``, for consistancy with `my_e3nn.o3.TensorProduct`.

    shared_weights : bool
        whether the `my_e3nn.o3.Linear` should be weighted individually for each input in a batch. Defaults to ``False``. Cannot be ``True`` if ``internal_weights`` is ``True``.
    ...
    ----------
    weight_numel : int
        the size of the weights for this `my_e3nn.o3.Linear`
    ...
    def __init__(
        ...
        *,
        ...
        opt_defaults = my_e3nn.get_optimization_defaults()
