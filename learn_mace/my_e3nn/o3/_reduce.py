...
from my_e3nn.math import germinate_formulas, orthonormalize, reduce_permutation
from my_e3nn.util import explicit_default_types
...
class ReducedTensorProducts(CodeGenMixin, torch.nn.Module):
    ...
    filter_ir_out : list of `my_e3nn.o3.Irrep`, optional
        Optional, list of allowed irrep in the output

    filter_ir_mid : list of `my_e3nn.o3.Irrep`, optional
        Optional, list of allowed irrep in the intermediary operations

    **kwargs : dict of `my_e3nn.o3.Irreps`
    ...
    Attributes
    ----------
    irreps_in : list of `my_e3nn.o3.Irreps`
    ...
    def __init__(self, formula, filter_ir_out=None, filter_ir_mid=None, eps=1e-9, **irreps):
        ...
        if filter_ir_out is not None:
            try:
                ...
                raise ValueError(f"filter_ir_out (={filter_ir_out}) must be an iterable of my_e3nn.o3.Irrep")

        if filter_ir_mid is not None:
            try:
                filter_ir_mid = [o3.Irrep(ir) for ir in filter_ir_mid]
            except ValueError:
                raise ValueError(f"filter_ir_mid (={filter_ir_mid}) must be an iterable of my_e3nn.o3.Irrep")
