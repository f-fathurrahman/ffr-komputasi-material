...
import MyMolecules: center_of_mass, translate
...
function find_point_group(mol::Molecule)
    ...
    if Ia_mol == 0.0
        # Linear Molecule
        if MyMolecules.isequivalent(MyMolecules.transform(mol, MyMolecules.inversion_matrix()), mol)
            return("Dinfh")
        else
            return("Cinfv")
        end
    elseif isapprox(Ia_mol, Ib_mol, atol=tol) && isapprox(Ia_mol, Ic_mol, atol = tol)
        ...
        SEAs = findSEA(D, 5)
        n = num_C2(mol, SEAs)
        invertable = MyMolecules.isequivalent(MyMolecules.transform(mol, MyMolecules.inversion_matrix()), mol)
        if n == 15
            return("Ih")
        elseif n == 9
            if invertable
                ...
                if c2_ortho
                    ...
                    if σh
                        ...
                        if σv
                            ...
                            S4 = MyMolecules.Sn(c2, 4)
                            molB = MyMolecules.transform(mol, S4)
            ...
            if q1
                ...
                if q2
                    ...
                    if q3
                        ...
                        S2n = MyMolecules.Sn(paxis, Cn*2)
                        molB = MyMolecules.transform(mol, S2n)
                        check = MyMolecules.isequivalent(mol, molB)
                        if check
