...
function find_rotations(mol::Molecule, rotation_set)
    ...
    out = []
    for i in rsi
        rmat = MyMolecules.Cn(i.axis, i.order)
        molB = MyMolecules.transform(mol, rmat)
        c = MyMolecules.isequivalent(molB, mol)
        if c
            push!(out, i)
        end
    end
...
function c2a(mol, sea)
    ...
    for i = 1:len-1, j = i+1:len
        ...
        if midpoint == [0.0, 0.0, 0.0]
            ...
            c2 = MyMolecules.Cn(midpoint, 2)
            molB = MyMolecules.transform(mol, c2)
            check = MyMolecules.isequivalent(mol, molB)
            if check
                return midpoint
            else
                continue
...
function c2c(mol, sea1, sea2)
    ...
    c2 = MyMolecules.Cn(c2_axis, 2)
    molB = MyMolecules.transform(mol, c2)
    if MyMolecules.isequivalent(mol, molB)
        return c2_axis
    else
        return nothing
    end
...
function c2c(mol, cn_axis, sea1, sea2)
    ...
    if issame_axis(c2_axis, cn_axis)
        ...
        c2 = MyMolecules.Cn(c2_axis, 2)
        molB = MyMolecules.transform(mol, c2)
        return MyMolecules.isequivalent(mol, molB)
    end
end
...
function is_there_sigmah(mol::Molecule, paxis)
    ...
    """
    σh = MyMolecules.reflection_matrix(paxis)
    molB = MyMolecules.transform(mol, σh)
    return MyMolecules.isequivalent(mol, molB)
end
...
function is_there_sigmav(mol, SEAs, paxis)
    ...
    for sea in SEAs
        ...
        for i = 2:len
            ...
            σ = MyMolecules.reflection_matrix(n)
            molB = MyMolecules.transform(mol, σ)
            check = MyMolecules.isequivalent(mol, molB)
            if check
                push!(axes, n)
            else
