...
```
julia> ints = MyFermi.Integrals.IntegralHelper()
...
function IntegralHelper(x...;k...)
    ...
    elseif precision == "double"
        ...
        throw(MyFermiException("precision can only be `single` or `double`. Got $precision"))
    end
end
...
function string_repr(X::IntegralHelper{T,E,O}) where {T,E,O}
    eri_string = replace("$E", "MyFermi.Integrals."=>"")
    orb_string = replace("$O", "MyFermi.Orbitals."=>"")
    out = ""
    out = out*" ⇒ MyFermi IntegralHelper\n"
    out = out*" ⋅ Data Type:                 $(T)\n"
...
function string_repr(X::AbstractDFERI)
    eri_string = replace("$(typeof(X))", "MyFermi.Integrals."=>"")
