...
Module to manage options in MyFermi. 

# Functions

    MyFermi.Options.set(option, value)     Set an <option> to a given <value>
    MyFermi.Options.get(option)            Return the current value of an <option>
    MyFermi.Options.reset()                Reset all options to default values
    MyFermi.Options.reset(option)          Reset a specific <option> to its default value
    MyFermi.Options.molecule(molstring)    Read in a String for the `molstring` option
...
"""
module Options
export MyFermiException

"""
    MyFermi.Options.Default

Dictionary containing default options for MyFermi. Any information not given
...
Dictionary containing user defined options for MyFermi. Unspecified options are obtained from 
MyFermi.Default.
"""
...
function get(key::String)
    ...
    elseif haskey(Default, key)
        ...
        throw(MyFermiException(key*" is not a valid option."))
    end
end


"""
    MyFermi.Options.set(key, val)
    ...
    if !isa(val, dtype)
        ...
        if dtype !== String
            try newval = dtype(val)
                ...
                throw(MyFermiException("Impossible to convert data type for $key. Expected: $dtype, Got: $(typeof(val))"))
            end
        end
        # Otherwise, throw an error
        throw(MyFermiException("Invalid data type for $key. Expected: $dtype, Got: $(typeof(val))"))
    else
        Current[key] = val
    end
    get(key)
...
"""
    MyFermi.Options.MyFermiException

Error flag used when MyFermi encounters an error. Use to give constructive feedback.
"""
struct MyFermiException <: Exception
    msg::String
end
Base.showerror(io::IO, e::MyFermiException) = print(io, "MyFermi found an Error: ", e.msg)
end #module
...
macro get(opt)
    ...
    quote
        MyFermi.Options.get($A)
    end |> esc
end

macro get()
    quote
        Keys = collect(keys(MyFermi.Options.Current))
        l = length(Keys)
        if l == 0
...
Set options for MyFermi computations. It saves the options into MyFermi.CurrentOptions.
...
macro set(block::Expr)
    ...
    for line in lines
        ...
        length(args) == 2 || throw(MyFermiException("Too many arguments for @set: $line"))
        ...
        if valtype <: Union{Number, Bool, String}
            push!(out.args, quote MyFermi.Options.set($key, $val) end)
        ...
        elseif valtype === Expr
            ...
            throw(MyFermiException("Invalid input for $key: $val"))
        end
    end
...
macro molecule(block)
    ...
    mol = String(mol)
    quote
        MyFermi.Options.set("molstring", $mol)
    end
end
...
Look up valid keywords for MyFermi.
...
macro lookup(A::Symbol)
    ...
    quote
        Keys = collect(keys(MyFermi.Options.Default))
...
macro freezecore()
    quote 
        mol = MyFermi.MyMolecule()
        atoms = mol.atoms
        # Dcore maps ranges of atomic numbers to number of core electrons
        Dcore = MyFermi.PhysicalConstants.core_elec_perrow
        Ncore = 0
        for a in atoms
            for k in keys(Dcore)
        ...
        Ncore = Ncore >> 1
        MyFermi.Options.set("drop_occ", Ncore)
    end
end
