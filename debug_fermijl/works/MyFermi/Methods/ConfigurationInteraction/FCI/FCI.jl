...
"""
    MyFermi.ConfigurationInteraction.FCIAlgorithm
...
"""
    MyFermi.ConfigurationInteraction.RFCI
...
Equivalent to
```
julia> MyFermi.ConfigurationInteraction.RFCI()
```
This function calls a constructor that runs a RFCI computation based on the options found in `MyFermi.Options`.
...
"""
    MyFermi.CoupledCluster.get_rfci_alg
...
function get_rfci_alg(N::Int = Options.get("ci_alg"))
    try 
        ...
        throw(MyFermiException("implementation number $N not available for RFCI."))
    end
end
...
function RFCI(x...)
    if !any(i-> i isa FCIAlgorithm, x)
        ...
        args = args[1:end-2]*")"
        throw(MyFermiException("invalid arguments for RFCI method: $args"))
    end
end
...
function string_repr(X::RFCI)
    out = ""
    out = out*" ⇒ MyFermi Restricted FCI Wave function\n"
