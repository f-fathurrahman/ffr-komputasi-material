...
function RFCI(alg::RFCIa)
    ...
    rhf = MyFermi.HartreeFock.RHF(aoints)
...
function RFCI(moints, aoints, alg::RFCIa)
    ...
    t = @elapsed begin
        hp  = MyFermi.Integrals.compute!(moints, aoints, "T")
        hp += MyFermi.Integrals.compute!(moints, aoints, "V")
        eri = MyFermi.Integrals.compute!(moints, "ERI")
    end
    ...
    Efci = klv[1][1] + MyMolecules.nuclear_repulsion(mol.atoms)
