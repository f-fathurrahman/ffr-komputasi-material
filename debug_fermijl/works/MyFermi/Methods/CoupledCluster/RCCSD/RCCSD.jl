...
"""
    MyFermi.CoupledCluster.RCCSDAlgorithm
...
Equivalent to
```
julia> MyFermi.CoupledCluster.RCCSD()
```
This function calls a constructor that runs a RCCSD computation based on the options found in `MyFermi.Options`.
...
"""
    MyFermi.CoupledCluster.get_rccsd_alg
...
function get_rccsd_alg(N::Int = Options.get("cc_alg"))
    try 
        ...
        throw(MyFermiException("implementation number $N not available for RCCSD."))
    end
end
...
function RCCSD(x...)
    if !any(i-> i isa RCCSDAlgorithm, x)
        ...
        args = args[1:end-2]*")"
        throw(MyFermiException("invalid arguments for RCCSD method: $args"))
    end
end
...
function string_repr(X::RCCSD)
    out = ""
    out = out*" ⇒ MyFermi Restricted CCSD Wave function\n"
