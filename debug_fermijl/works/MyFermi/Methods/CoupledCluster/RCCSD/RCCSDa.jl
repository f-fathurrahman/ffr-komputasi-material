...
function RCCSD(moints::IntegralHelper{T,<:AbstractERI,<:AbstractRestrictedOrbitals}, aoints::IntegralHelper{T,<:AbstractERI,AtomicOrbitals}, alg::RCCSDa) where T<:AbstractFloat
    ...
    MyFermi.Integrals.compute!(moints, aoints, "OOVV")
...
function RCCSD(moints::IntegralHelper{T,E1,<:AbstractRestrictedOrbitals}, aoints::IntegralHelper{T,E2,AtomicOrbitals}, alg::RCCSDa) where {T<:AbstractFloat,T2<:AbstractFloat,
    ...
    MyFermi.Integrals.compute!(moints, aoints, "BVV")
    RCCSD(moints, alg)
end
...
function RCCSD(moints::IntegralHelper{T,E,O}, newT1::AbstractArray{T,2}, newT2::AbstractArray{T,4}, 
    ...
    do_diis ? DM_T1 = MyFermi.DIIS.DIISManager{T, diis_prec}(size=ndiis) : nothing
    do_diis ? DM_T2 = MyFermi.DIIS.DIISManager{T, diis_prec}(size=ndiis) : nothing
    ...
    if do_diis
        DM_T1 = MyFermi.DIIS.DIISManager{T, diis_prec}(size=ndiis)
        DM_T2 = MyFermi.DIIS.DIISManager{T, diis_prec}(size=ndiis)
    end
        ...
        t = @elapsed begin
            ...
            if do_diis
                ...
                if ite ≥ diis_start && ite % diis_relax == 0
                    ...
                    newT2 = MyFermi.DIIS.extrapolate(DM_T2;add_res=true)
                    newT1 = MyFermi.DIIS.extrapolate(DM_T1;add_res=true)
                    diis = true
                end
            end
