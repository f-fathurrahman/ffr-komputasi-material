...
function calcJK!(J, K, D, ints::IntegralHelper{Float64,<:SparseERI,AtomicOrbitals})
    ...
    Threads.@threads for z = eachindex(eri_vals)
        ...
        @inbounds begin
            @fastmath begin
            ij = MyFermi.index2(i-1,j-1)
            ik = MyFermi.index2(i-1,k-1) + 1
            il = MyFermi.index2(i-1,l-1) + 1

            jk = MyFermi.index2(j-1,k-1) + 1
            jl = MyFermi.index2(j-1,l-1) + 1

            kl = MyFermi.index2(k-1,l-1) 
