...
|:------------|:-----------------------------------|:-----------------------------------------------------------|
|`atoms`      | `Vector{Atom}`                     | An array of `MyMolecules.Atom` objects  |
...
```julia
julia> water = MyMolecules.parse_string(
...
```julia
julia> h2 = MyMolecules.parse_string(
   "H 0.0 0.0 0.0
    H 0.0 0.0 0.7"
)
...
function BasisSet(name::String, str_atoms::String)
    atoms = MyMolecules.parse_string(str_atoms)
    BasisSet(name, atoms)
end
...
"""
    MyGaussianBasis.gto_norm(n::Signed, a::AbstractFloat)
...
"""
    MyGaussianBasis.normalize_basisfunction!(B::BasisFunction)
