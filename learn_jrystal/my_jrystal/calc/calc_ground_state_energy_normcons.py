...
def calc(config: JrystalConfigDict) -> GroundStateEnergyOutput:
    ...
    print("<div> Creating crystal")
    crystal = create_crystal(config)
    print("</div> End creating cystal")

    print("<div> Creating crystal")
    pseudopot = create_pseudopotential(config)
    print("</div> End creating crystal")

    valence_charges = np.sum(pseudopot.valence_charges)
    print(f"Crystal: {crystal.symbols}")
    EPS = config.eps
    ...
    print(f"Parallel over k-mesh: {config.parallel_over_k_mesh}.")
    ...
    print(f"Number of G-vectors: {proper_grid_size(config.grid_sizes)}")
    print(f"Number of k-vectors: {proper_grid_size(config.k_grid_sizes)}")
    ...
    print("Initializing pseudopotential (local)...")
    start = time.time()
    ...
    print("Initializing pseudopotential (Spherical Bessel Transform)...")
    start = time.time()
    ...
    print("Initializing pseudopotential (nonlocal)...")
    start = time.time()
    ...
    print(f"Nonlocal potential done. Times: {end - start:.2f} seconds")
    print("Deploying pseudopotential (nonlocal)...")
    start = time.time()
    ...
    with mesh:
        ...
        print(f"smearing: {config.smearing}")
    ...
    hartree = energy.hartree(density_reciprocal, g_vec, crystal.vol)
    external_local = normcons.energy_local(
        density_reciprocal, potential_loc, vol=crystal.vols
    )
    external_nonlocal = normcons.energy_nonlocal(
        coeff, potential_nl, vol=crystal.vol, occupation=occ
    )
    ...
    print(f"Hartree Energy: {hartree:.4f} Ha")
    print(f"External (local) Energy: {external_local:.4f} Ha")
    print(f"External (nonlocal) Energy: {external_nonlocal:.4f} Ha")
    print(f"XC Energy: {xc:.4f} Ha")
    print(f"Kinetic Energy: {kinetic:.4f} Ha")
    print(f"Nuclear repulsion Energy: {ew:.4f} Ha")
    print(f"Total Energy: {etot+ew:.4f} Ha")
