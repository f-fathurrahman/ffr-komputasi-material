...
! AOLD= matrix inverse referring to the old determinant for each spin
       ...
       public :: SLASM,SLAKIN,DETARR

      contains
!-----------------------------------------------------------------------
      subroutine INITDET
!  Used instead of a stepwise generation of the feterminant at start
!  from simple values as unity matrix etc..
       integer  :: j,hs
       hs=NES(1)
       AOLD=0.0_dp
       ANEW=0.0_dp
...
!  planew= laplacian of one-electron wavefunction at new position
       real(dp),intent(in),dimension(NES(IES),NES(IES)) :: aold
       real(dp),intent(in),dimension(3,NE)            :: pgrnew
       real(dp),intent(in),dimension(NE)              :: planew
...
!
       ...
       hpla = 0.0_dp
       do js=1,ns
        hpgr(1:3) = hpgr(1:3) + aold(IEES,js)*pgrnew(1:3,ns0+js)
        hpla = hpla + aold(IEES,js)*planew(ns0+js)
       end do
     ...
     &     (dabs(hpla) > 1000.0_dp)) toolarge = .true.
       ...
       if (toolarge) write(39,*) 'kinetic energy large grad,lapl=', abs(sum(hpgr)),hpla
