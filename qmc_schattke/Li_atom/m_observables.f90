...
! KINEN(IE)=KINENOLD(IE)
! VELEN(IE)=VELENOLD(IE)
...
! Need this?
!         KINENOLD(IE)=KINEN(IE)
!         VELENOLD(IE)=VELEN(IE)
...
subroutine POTENERGY(poten)
  ! Calculates one-particle potential energy.
  real(dp),intent(out),dimension(NE)  :: poten
  real(dp)  ::  r
  r = dsqrt(sum(RENEW(1:3,IE)**2))
  r = max (r,JASEMACH)
  poten(IE) = -3.0_dp/r
...
!  Calculates kinetic energy kinen(IE) of electron IE.
       ...
       real(dp),dimension(3,NORB)            :: pgrnew
       real(dp),dimension(NORB)              :: psinew,planew
       if (MCSTEP) then
       qsel = QD(IES)
        call ORBDER(RENEW(1:3,IE),psinew,pgrnew,planew)
        hgrj(1:3)=GRJAS(1:3,IEES,IES)
        hlaj=LAPJAS(IEES,IES)
       else
  qsel = 1._dp
        call ORBDER(RE(1:3,IE),psinew,pgrnew,planew)
...
! !!! error correction below by "velen --> velen(IE)" 
