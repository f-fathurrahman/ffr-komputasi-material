...
function Atoms(sys::AtomsBase.AbstractSystem)
   ...
   return MyJuLIP.Atoms(X, M .* V, M, Z, hcat(cell...)', pbc; data=data)
end
...
function Base.show(io::IO, a::Atoms)
   print(io, "MyJuLIP.Atoms with $(length(a)) atoms")
end
...
MyJuLIP.FIO.write_dict(at::Atoms) =
   Dict( "__id__" => "MyJuLIP_Atoms",
         "X"      => at.X,
         "P"      => at.P,
         "M"      => at.M,
         "Z"      => Int.(at.Z),
...
MyJuLIP.FIO.read_dict(::Val{:MyJuLIP_Atoms}, D::Dict) = Atoms(D)
...
# define conversions to be applied when reading from ExtXYZ files to MyJuLIP Atoms
...
`file` can be a string filename, open IO stream or `FILE*` pointer.
`atoms` can be either a single `MyJuLIP.Atoms` or a vector of `MyJuLIP.Atoms`.
"""
