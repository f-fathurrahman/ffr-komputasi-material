...
import MyJuLIP: read_dict, write_dict
export @D, @DD, @GRAD, @pot

using MyJuLIP: AtomicNumber
...
function z2i(Zs::AbstractZList, z::AtomicNumber)
   if Zs.list[1] == MyJuLIP.Chemistry.__zAny__
      return 1
   end
...
write_dict(zlist::ZList) = Dict("__id__" => "MyJuLIP_ZList",
                                  "list" => Int.(zlist.list))

read_dict(::Val{:MyJuLIP_ZList}, D::Dict) = ZList(D)
ZList(D::Dict) = ZList(D["list"])

write_dict(zlist::SZList) = Dict("__id__" => "MyJuLIP_SZList",
                                 "list" => Int.(zlist.list))
read_dict(::Val{:MyJuLIP_SZList}, D::Dict) = SZList(D)
