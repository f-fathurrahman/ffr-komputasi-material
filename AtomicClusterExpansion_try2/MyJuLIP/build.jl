...
import MyJuLIP
using ..Chemistry
using MyJuLIP: JVec, JMat, JVecF, JMatF, mat, vecs,
...
function cluster(atu::Atoms{T}, R::Real;
   ...
   if !isdiag(cell(atu))
      if length(atu) > 1
         error("""`MyJuLIP.cluster` requires as argument either a cubic cell or a
                  one-atom cell.""")
      end
      atu = _cubic_cell(atu)
   end
