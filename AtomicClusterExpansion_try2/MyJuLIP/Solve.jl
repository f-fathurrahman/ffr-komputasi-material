...
using MyJuLIP: AbstractAtoms, update!,
             dofs, energy, gradient, set_dofs!, site_energies,
             Dofs, calculator, AbstractCalculator, r_sum, fixedcell

using MyJuLIP.Potentials: SitePotential
using MyJuLIP.Preconditioners: Exp
...
function minimise!(at::AbstractAtoms;
   ...
   elseif method == :sd
      ...
      error("MyJuLIP.Solve.minimise!: unknown `method` option")
   end
