...
write_dict(z::AtomicNumber) =
      Dict("__id__" => "MyJuLIP_AtomicNumber", "z" => z.z)

read_dict(::Val{:MyJuLIP_AtomicNumber}, D::Dict) =
      AtomicNumber(D["z"])

#data = load_dict(@__DIR__()[1:end-3] * "data/asedata.json")
data = load_dict(joinpath(pwd(), "my_julip_data/asedata.json")) #XXX ffr: This is not robust
