...
using MyJuLIP:       AbstractCalculator, AbstractAtoms, JVec, AtomicNumber, JMat

import MyJuLIP:      energy, forces, virial, site_energy, site_energy_d,
                   alloc_temp, alloc_temp_d, evaluate, evaluate_d,
                   evaluate!, evaluate_d!, evaluate_ed,
                   read_dict, write_dict, fltype, rfltype

import MyJuLIP.Potentials: site_virial

import MyJuLIP: combine
...
write_dict(coll::IPCollection) = Dict(
            "__id__" => "MyJuLIP_IPCollection",
            "coll" => Dict.(coll.coll) )
IPCollection(D::Dict) = IPCollection( read_dict.( D["coll"] ) )
read_dict(::Val{:MyJuLIP_IPCollection}, D::Dict) = IPCollection(D)
...
# -------------------------------------------------------------
#       MyJuLIP Calculator functionality for IPBasis
# -------------------------------------------------------------

using NeighbourLists: maxneigs
using MyJuLIP: sites, neighbourlist, cutoff, JVec
using MyJuLIP.Potentials: neigsz!
