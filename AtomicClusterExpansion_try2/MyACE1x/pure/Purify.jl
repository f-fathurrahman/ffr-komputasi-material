...
using MyACE1x: getspeclenlist, spec2col, insertspect!

using SparseArrays: sparse, spzeros, SparseVector
using RepLieGroups.O3: ClebschGordan

const NLMZ = MyACE1.RPI.PSH1pBasisFcn

function pureRPIBasis(basis::MyACE1.RPIBasis; remove = 0)
   ...
   for C_sym in C_sym_list
       for idx = 1:size(C_sym, 1)
           ...
           spec_iAA = MyACE1.get_basis_spec(basis.pibasis, 1, iAA).oneps
   ...
   spec_list = [MyACE1.get_basis_spec(basis.pibasis, i) for i in eachindex(zList)]
   maxn_list = []
   maxl_list = []
   ...
   if Rn.trans isa PolyTransform
      Rn_x = MyACE1.OrthPolys.transformed_jacobi(maxn+ninc, Rn.trans, Rn.ru, Rn.rl; pcut = pcut, pin = pin)
   elseif Rn.trans isa Transforms.MultiTransform
      Rn_x = MyACE1.transformed_jacobi(maxn+ninc, Rn.trans; pin = pin, pcut = pcut)
   else
   ...
   NN2b = MyACE1x.getNN2b(maxn, ninc, pin, pcut)
   Rn_coef = MyACE1x.Rn_prod_coeffs(Rn_x.J, NN2b)

   # === 
   # Define extended 1 particle basis in ACEcore, and convert it to MyACE1 format
   spec1p_x_core = [(z, i, y) for z in [list.z for list in zList] for i = 1:(maxn + ninc) for y = 1:Ydim]
   spec1p_x = MyACE1x.getMyACE1Spec1p_multi(spec1p_x_core)
   ...
   spec_core_list = MyACE1x.getACEcoreSpec_1px_multi(basis, spec1p_x_core, zList)

   # Sanity check
   for t in eachindex(zList)
       @assert length(spec_core_list[t]) == length(MyACE1.get_basis_spec(basis.pibasis, t))
   end
   ...
   for i in eachindex(zList)
      ...
      spec = MyACE1.get_basis_spec(basis.pibasis, i)
      ...
      # get spec_x in terms of MyACE1 spec
      spec_x = MyACE1x.getMyACE1Spec_multi(spec_x_core, spec1p_x_core, species[i])
   ...
   # thin_spec1p_x = Vector{MyACE1.RPI.PSH1pBasisFcn}()
   ...
   pibasis_x = MyACE1.pibasis_from_specs(b1p_x, Tuple(spec_x_list))

   # symmetrize it
   pure_rpibasis = RPIBasis(pibasis_x, Tuple(newA2Bmap_list), basis.Bz0inds)

   # delete zero basis functions
   pure_rpibasis = MyACE1.RPI.remove_zeros(pure_rpibasis)

   # and finally clean up
   pure_rpibasis = MyACE1._cleanup(pure_rpibasis)
   ...
   for nlm in spec3b
      if !(nlm in keys(Cnn_all))
         ...
         if z1 == z2
            ...
            (l1, m1), (l2, m2) = MyACE1x.idx2lm(niceidx1[3]), MyACE1x.idx2lm(niceidx2[3])
            ...
            for κ = 1:length(Pκk1k2.nzind)
               for L = abs(M):(l1 + l2)
                     C_nn[MyACE1x.LinearSearch((z1, Pκk1k2.nzind[κ], MyACE1x.index_y(L, M)), spec1p)] = Pκk1k2.nzval[κ] * sqrt( (2*l1+1)*(2*l2+1) / (4 * π * (2*L+1)) ) *
                     cg(l1,  0, l2,  0, L, 0) *
                     cg(l1, m1, l2, m2, L, M)
               end
            end
