...
using MyACE1: gensparse, rand_radial, PIBasisFcn
...
# ========================================== utils for converting between MyACE1 and ACEcore interface ===========================

"""
Get a list of spec in ACEcore style from MyACE1 pibasis in spec1p_x_core index corresponding to different Atoms
...
function getACEcoreSpec_1px_multi(rpiB, spec1p_x_core, zList)
    ...
    old_spec_list = [MyACE1.get_basis_spec(old_pibasis, i) for i in eachindex(zList)]
...
"""
Get spec in MyACE1 format for multi species
"""
function getMyACE1Spec_multi(ac_spec, spec1p_core, center_species)
    zX = AtomicNumber(center_species)
    ace1_spec = Vector{PIBasisFcn{N, MyACE1.RPI.PSH1pBasisFcn} where N}()
...
"""
Get spec1p in MyACE1 format for multi species, this just ignore the species channel.
"""
function getMyACE1Spec1p_multi(ac_spec1p)
    ace1_spec1p = Vector{MyACE1.RPI.PSH1pBasisFcn}()
    for t in ac_spec1p
