...
import YAML, MyACE1, MyJuLIP
using NamedTupleTools: namedtuple
using MyJuLIP: AtomicNumber, rnn, z2i
using MyACE1.Transforms: agnesi_transform, multitransform, 
                       normalized_agnesi_transform
using MyACE1.PairPotentials: PolyPairBasis
using MyACE1.OrthPolys: transformed_jacobi, transformed_jacobi_env

export ace_basis, smoothness_prior, ace_defaults 

# -------------- Bond-length heuristics

#_lengthscales_path = joinpath(@__DIR__, "..", "data", "length_scales_VASP_auto_length_scales.yaml")
_lengthscales_path = joinpath(pwd(), "my_ace1x_data", "length_scales_VASP_auto_length_scales.yaml")
...
function _clean_args(kwargs)
   ...
   for key in keys(kwargs)
      if haskey(MyACE1x._kw_aliases, key)
         dargs[MyACE1x._kw_aliases[key]] = kwargs[key]
      else
...
function _get_degrees(kwargs)
   ...
   if haskey(kwargs, :totaldegree)
      ...
      if deg isa Union{AbstractVector{<: Number}, Number}
         Deg, maxdeg = MyACE1.Utils._auto_degrees(cor_order, deg,
                                                kwargs[:wL], nothing)
         maxn = maximum(deg)
...
function _transform(kwargs; transform = kwargs[:transform])
   ...
   elseif transform isa MyACE1.Transforms.DistanceTransform
      rcut = _get_all_rcut(kwargs)
      rcut = maximum(values(rcut))  # multitransform wants a single cutoff.
...
function algebraic_smoothness_prior(basis; p = 4, wL = 1.0) 
   d = Float64[]
   for B in basis.BB
      if B isa MyACE1.RPI.RPIBasis
         append!(d, MyACE1.scaling(B, p, wL))
      else
         append!(d, MyACE1.scaling(B, p))
      end
   end
   return Diagonal(1 .+ d)
end
...
_get_nnll(basis::MyACE1.RPI.RPIBasis) = 
      MyACE1.get_nl(basis)

_get_nnll(basis::MyACE1.PolyPairBasis) = 
      [ [(n = n, l = 0),] for n in MyACE1.scaling(basis, 1) ]
