...
using MyACE1: SparsePSHDegree, BasicPSH1pBasis, PIBasis, get_basis_spec, 
            PIBasisFcn, pibasis_from_specs, degree, 
            rand_radial, order, evaluate, i2z, z2i, numz
using MyACE1.RPI: RPIBasis, RPIFilter   
...
function pure2b_basis(; species = nothing, Rn = nothing,
   ...
   rpibasis = MyACE1.RPI.remove_zeros(rpibasis)
   # and finally cleanup the rest of the basis 
   rpibasis = MyACE1._cleanup(rpibasis)
