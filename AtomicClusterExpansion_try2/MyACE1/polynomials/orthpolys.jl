...
import MyJuLIP: evaluate!, evaluate_d!, JVec, cutoff, fltype
import MyJuLIP.FIO: read_dict, write_dict
import MyJuLIP.MLIPs: alloc_B, alloc_dB, IPBasis

import MyACE1
using MyACE1.Transforms: DistanceTransform, transform, transform_d,
...
struct OrthPolyBasis{T} <: MyACE1.ScalarBasis{T}
...
read_dict(::Val{:MyACE1_OrthPolyBasis}, D::Dict) = OrthPolyBasis(D)
...
function MyACE1.rand_radial(J::OrthPolyBasis)
...
struct TransformedPolys{T, TT, TJ, TENV} <: MyACE1.ScalarBasis{T}
...
write_dict(J::TransformedPolys) = Dict(
      "__id__" => "MyACE1_TransformedPolys",
      "J" => write_dict(J.J),
      "rl" => J.rl,
      "ru" => J.ru,
...
read_dict(::Val{:MyACE1_TransformedPolys}, D::Dict) = TransformedPolys(D)

Base.length(J::TransformedPolys) = length(J.J)
fltype(P::TransformedPolys{T}) where {T} = T

function MyACE1.rand_radial(J::TransformedPolys, z1, z2)
   t = MyACE1.rand_radial(J.J)
   return inv_transform(J.trans, t, z1, z2)
end

function MyACE1.rand_radial(J::TransformedPolys)
   t = MyACE1.rand_radial(J.J)
...
function transformed_jacobi(maxdeg::Integer,
   ...
   @assert eltype(trans.transforms) <: MyACE1.Transforms.AffineT
   @assert all( (t.y1 == -1) && (t.y2 == 1) 
                 for t in trans.transforms )
   # obtain the maximum outer cutoff and minimum inner cutoff 
   rin, rcut = MyACE1.Transforms.cutoff_extrema(trans)
...
import MyACE1: evaluate, evaluate_d
import ForwardDiff
...
write_dict(env::OneEnvelope) = Dict(
      "__id__" => "MyACE1_OneEnvelope",
   )

read_dict(::Val{:MyACE1_OneEnvelope}, D::Dict) = OneEnvelope()
...
write_dict(env::PolyEnvelope{T}) where {T} = Dict(
      "__id__" => "MyACE1_PolyEnvelope",
      "T" => write_dict(T), 
      "p" => env.p,
      "r0" => env.r0,
      "rcut" => env.rcut, )

function read_dict(::Val{:MyACE1_PolyEnvelope}, D::Dict) 
   T = read_dict(D["T"])
...
write_dict(env::TwoSidedEnvelope{T}) where {T} = Dict(
      "__id__" => "MyACE1_TwoSidedEnvelope",
      "T" => write_dict(T),
      "xl" => env.xl,
      "xr" => env.xr,
      "pl" => env.pl,
...
function read_dict(::Val{:MyACE1_TwoSidedEnvelope}, D::Dict) 
   T = read_dict(D["T"])
