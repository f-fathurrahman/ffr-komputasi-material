...
import Base:   ==
import MyJuLIP:  cutoff, AtomicNumber
import MyJuLIP.FIO: read_dict, write_dict
import MyACE1: allfieldsequal
...
write_dict(T::PolyTransform) =
   Dict("__id__" => "MyACE1_PolyTransform", 
             "p" => T.p, 
            "r0" => T.r0, 
             "a" => T.a)
...
read_dict(::Val{:MyACE1_PolyTransform}, D::Dict) = PolyTransform(D)
...
write_dict(T::IdTransform) =  Dict("__id__" => "MyACE1_IdTransform")
IdTransform(D::Dict) = IdTransform()
read_dict(::Val{:MyACE1_IdTransform}, D::Dict) = IdTransform(D)
...
write_dict(T::MorseTransform) =
   Dict("__id__" => "MyACE1_MorseTransform", "lambda" => T.p, "r0" => T.r0)
MorseTransform(D::Dict) = MorseTransform(D["lambda"], D["r0"])
read_dict(::Val{:MyACE1_MorseTransform}, D::Dict) = MorseTransform(D)
...
write_dict(T::AgnesiTransform) =
      Dict("__id__" => "MyACE1_AgnesiTransform", 
           "r0" => T.r0, "p" => T.p, "a" => T.a, "rin" => T.rin)

AgnesiTransform(D::Dict) = AgnesiTransform(; r0=D["r0"], p=D["p"], 
                                             a=D["a"], rin=D["rin"])

read_dict(::Val{:MyACE1_AgnesiTransform}, D::Dict) = AgnesiTransform(D)
...
write_dict(T::Agnesi2Transform) =
      Dict("__id__" => "MyACE1_Agnesi2Transform", 
           "r0" => T.r0, "p" => T.p, "q" => T.q, "a" => T.a, "rin" => T.rin)

Agnesi2Transform(D::Dict) = Agnesi2Transform(D["r0"], D["p"], D["q"], 
                                             D["a"], D["rin"])

read_dict(::Val{:MyACE1_Agnesi2Transform}, D::Dict) = Agnesi2Transform(D)
...
import MyJuLIP
import MyJuLIP.Potentials: ScalarFun
...
Example: 
```julia
using MyACE1: AnalyticTransform
...
function AnalyticTransform(str_f::String, str_finv::String; T=Float64)
   ...
   ex_df = MyJuLIP.Potentials.fdiff( ex_f, 1 )
   if str_finv == "auto"
...
write_dict(T::AnalyticTransform) =
         Dict("__id__" => "MyACE1_AnalyticTransform", 
              "f" => T.str_f, "finv" => T.str_finv)
AnalyticTransform(D::Dict) = AnalyticTransform(D["f"], D["finv"])
read_dict(::Val{:MyACE1_AnalyticTransform}, D::Dict) = AnalyticTransform(D)
...
write_dict(T::AffineT) = 
      Dict("__id__" => "MyACE1_AffineT", 
           "t" => write_dict(T.t), 
           "xy" => [T.x1, T.x2, T.y1, T.y2] )

read_dict(::Val{:MyACE1_AffineT}, D::Dict) = AffineT(D) 
...
import MyJuLIP: chemical_symbol
import MyJuLIP.Potentials: ZList, SZList, i2z, z2i
...
write_dict(T::MultiTransform) =
      Dict("__id__" => "MyACE1_MultiTransform", 
           "zlist" => write_dict(T.zlist), 
           "transforms" => write_dict.(T.transforms[:]))

read_dict(::Val{:MyACE1_MultiTransform}, D::Dict) = MultiTransform(D)
