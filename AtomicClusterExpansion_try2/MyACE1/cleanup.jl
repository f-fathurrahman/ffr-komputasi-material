...
function _cleanup(basis::MyJuLIP.MLIPs.IPSuperBasis)
   BB_new = _cleanup.(basis.BB)
   return MyJuLIP.MLIPs.IPSuperBasis(BB_new)
end
...
function _cleanup(pibasis::PIBasis, IInz, IIz)
   ...
   basis1p_new = MyACE1.RPI.BasicPSH1pBasis(J_new, SH_new, basis1p.zlist, spec_new, Ainds_new)
   ...
   for iz = 1:NZ 
      ...
      for (i, b) in enumerate(basis1p_new.spec)
         bz = MyACE1.RPI.PSH1pBasisFcn(b.n, b.l, b.m, z)
