...
import MyACE1: standardevaluator, graphevaluator
...
# v0.8.2 onwards
function read_dict(::Val{:MyACE1_RPIBasis}, ::Val{:v0_8_2}, D::Dict)
...
# old version
read_dict(::Val{:MyACE1_RPIBasis}, D::Dict) =
   ...
   for i = 1:length(pibasis.inner[iz0])
      ...
      for irow = 1:size(U, 1)
         ...
         for (icol, bcol) in enumerate(bcols)
            ...
            bcol_ordered = MyACE1._get_ordered(pibasis, bcol)
