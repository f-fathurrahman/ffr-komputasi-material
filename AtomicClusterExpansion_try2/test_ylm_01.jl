...
##
import MyACE1
using MyJuLIP.Testing
using LinearAlgebra, StaticArrays, BenchmarkTools, Test, Printf
using MyACE1.SphericalHarmonics
using MyACE1.SphericalHarmonics: dspher_to_dcart, PseudoSpherical,
               cart2spher, spher2cart
using MyJuLIP: evaluate, evaluate_d, evaluate_ed
...
for nsamples = 1:30
   ...
   θ = 0.1+0.4 * pi * rand()
   L = 5
   P = MyACE1.SphericalHarmonics.compute_p(L, θ)
   P1, dP = MyACE1.SphericalHarmonics.compute_dp(L, θ)
   # -------------
   P_eq_P1 = true
   for l = 0:L, m = 0:l
      i = MyACE1.SphericalHarmonics.index_p(l, m)
   ...
   for p = 2:10
      h = 0.1^p
      dPh = (MyACE1.SphericalHarmonics.compute_p(L, θ+h) - P) / h
...
for nsamples = 1:30
   ...
   L = 5
   P = MyACE1.SphericalHarmonics.compute_p(L, θ)
   _, dP = MyACE1.SphericalHarmonics.compute_dp(L, θ)
   errs = []
   verbose && @printf("     h    | error \n")
   for p = 2:10
      h = 0.1^p
