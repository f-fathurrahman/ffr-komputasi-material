import MyACEfit
import MyJuLIP: Atoms, energy, forces, mat
using PrettyTables
...
struct AtomsData <: MyACEfit.AbstractData
    atoms::Atoms
...
function MyACEfit.count_observations(d::AtomsData)
    return !isnothing(d.energy_key) +
           3*length(d.atoms)*!isnothing(d.force_key) +
           6*!isnothing(d.virial_key)
end

function MyACEfit.feature_matrix(d::AtomsData, basis; kwargs...)
    dm = Array{Float64}(undef, MyACEfit.count_observations(d), length(basis))
    i = 1
...
function MyACEfit.target_vector(d::AtomsData; kwargs...)
    y = Array{Float64}(undef, MyACEfit.count_observations(d))
    i = 1
    if !isnothing(d.energy_key)
    ...
    if !isnothing(d.virial_key)
        ...
        #   https://github.com/JuliaMolSim/MyJuLIP.jl/issues/166
...
function MyACEfit.weight_vector(d::AtomsData; kwargs...)
    w = Array{Float64}(undef, MyACEfit.count_observations(d))
    i = 1
    if !isnothing(d.energy_key)
...
function recompute_weights(raw_data;
    ...
    return MyACEfit.assemble_weights(data)
end
