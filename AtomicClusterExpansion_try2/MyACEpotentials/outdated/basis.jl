...
import MyACE1.PairPotentials: PolyPairBasis
import MyACE1.Transforms: PolyTransform, MultiTransform
...
required to construct an MyACE basis (`RPIBasis`). All parameters 
...
# Radial basis of MyACE
Returns a dictionary containing the complete set of parameters 
required to construct radial basis for MyACE. All parameters are 
...
complete set of parameters required to construct an MyACE basis (`RPIBasis`). 
...
"""Returns MyACE1.Utils.rpi_basis """
...
complete set of parameters required to construct radial basis for MyACE. 
All parameters are passed as keyword argument. 
...
function generate_radial_basis(params::Dict, D, maxdeg, species, trans)
   maxn = MyACE1.RPI.get_maxn(D, maxdeg, species)
...
to construct `MyACE1.RPI.SparsePSHDegree`. See `?SparsePSHDegree`.
...
##`SparsePSHDegreeM`
Returns a dictionary containing the complete set of parameters required 
to construct `MyACE1.RPI.SparsePSHDegree`. Also see `?SparsePSHDegreeM`. 

NB `maxdeg` of MyACE basis (`RPIBasis`) has to be set to `1.0`.
...
complete set of parameters required to construct `MyACE1.RPI.SparsePSHDegree`.
See `?SparsePSHDegree`.
...
SparsePSHDegreeM(; Dn::Dict, Dl::Dict, Dd::Dict) = 
      MyACE1.RPI.SparsePSHDegreeM(_params_to_AtomicNumber(Dn), 
                                _params_to_AtomicNumber(Dl), 
                                _params_to_AtomicNumber(Dd))

_degrees = Dict(
      "sparse" => (MyACE1.RPI.SparsePSHDegree, sparse_degree_params),
...
Returns a dictionary containing the complete set of parameters required 
to construct `MyACE1.Transforms.PolyTransform``. All parameters are passed
...
# Multitransform
Returns a dictionary containing the complete set of parameters required 
to construct `MyACE.Transform.multitransform`. All parameters are passed 
as keyword argument. 
...
needed to construct `MyACE1.Transforms.IdTransform`.  
"""
...
complete set of parameters required to construct `MyACE1.Transforms.PolyTransform``.
...
complete set of parameters required to construct `MyACE.Transform.multitransform`.
All parameters are passed as keyword argument. 
...
function generate_multitransform(; transforms, rin = nothing, rcut = nothing, cutoffs = nothing)
      ...
      return MyACE1.Transforms.multitransform(transforms, rin = rin, rcut = rcut, cutoffs = cutoffs)
end
...
# parameters 
_transforms = Dict(
      "polynomial" => (MyACE1.Transforms.PolyTransform, PolyTransform_params),
      "multitransform" => (generate_multitransform, multitransform_params),
      "identity" => (MyACE1.Transforms.IdTransform, IdTransform_params)
)
