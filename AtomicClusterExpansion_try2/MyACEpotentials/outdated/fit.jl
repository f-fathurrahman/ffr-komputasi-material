...
function fit_ace(params::Dict)
    ...
    basis = [MyACEpotentials.generate_basis(basis_params) for (basis_name, basis_params) in params["basis"]]
    basis = MyJuLIP.MLIPs.IPSuperBasis(basis);
    ...
    dataset = MyJuLIP.read_extxyz(params["data"]["fname"])
    ...
    # create solver
    solver = eval(Meta.parse("MyACEfit." * uppercase(params["solver"]["type"])))
    ...
    if isnothing(params["P"])
        ...
        P = MyACEpotentials.generate_regularizer(basis, params["P"])
    end
    fit = MyACEfit.linear_fit(data, basis, solver; P)
    C = fit["C"]
    IP = MyJuLIP.MLIPs.combine(basis, C)
    (Vref != nothing) && (IP = MyJuLIP.MLIPs.SumIP(Vref, IP))

    errors = linear_errors(data, IP)
    results = Dict{String,Any}("IP" => IP, "errors" => errors)

    save_fit(params["MyACE_fname"], IP, Dict("errors" => errors, "params" => params))

    if haskey(fit, "committee")
        IP_com = MyACE1.committee_potential(basis, C, fit["committee"])
        (Vref != nothing) && (IP_com = MyJuLIP.MLIPs.SumIP(Vref, IP_com))
        results["IP_com"] = IP_com
        if !(params["MyACE_fname"] == "") && !isnothing(params["MyACE_fname"])
            save_fit(chop(params["MyACE_fname"],tail=5)*"-committee.json", IP_com, Dict("errors" => errors, "params" => params))
        end
    end
...
parameters needed to make an MyACE potential. All parameters are passed 
as keyword argumts. 
...
* `MyACE_fname = "MyACE_fit.json"` : filename to save MyACE to. Potential & info
    do not get saved if `MyACE_fname` isnothing() or is set to `""`. Files already _parse_entry
...
#    solver = MyACEpotentials.generate_solver(params["solver"])
#
#    if !isnothing(params["P"]) 
#        solver["P"] = MyACEpotentials.generate_regularizer(db.basis, params["P"])
#    end
#
...
#
#    save_fit(params["MyACE_fname"], IP, lsqinfo)
#
#    @info("Fitting errors")
...
function make_ace_db(params::Dict)
    data =  MyACEpotentials.read_data(params["data"])
    basis = [MyACEpotentials.generate_basis(basis_params) for (basis_name, basis_params) in params["basis"]]
    basis = MyJuLIP.MLIPs.IPSuperBasis(basis);
