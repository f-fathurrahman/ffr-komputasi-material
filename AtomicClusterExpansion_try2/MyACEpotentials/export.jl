using MyACE1
using MyACE1: PIBasis, PIBasisFcn, PIPotential
using MyACE1: rand_radial, cutoff, numz, ZList
import Interpolations
using MyJuLIP: energy, bulk, i2z, z2i, chemical_symbol, SMatrix
...
function export2lammps(fname, IP)
    ...
    # decomposing into V1, V2, V3 (One body, two body and MyACE bases)
    ...
    radialsplines = MyACE1.Splines.RadialSplines(V3.pibasis.basis1p.J; nnodes = 10000)
    ranges, nodalvals, zlist = MyACE1.Splines.export_splines(radialsplines)
    ...
    functions, lmax = export_MyACE_functions(V3, species, reversed_species_dict)
...
make_dimer(s1, s2, rr) = Atoms(
    ...
    [MyJuLIP.atomic_mass(s1), MyJuLIP.atomic_mass(s2)],
...
function write_pairpot_table(fname, V2, species_dict)
    ...
    # make header. date is none since MyACE1 current doesnt depend on time/dates package
    push!(lines, "# DATE: none UNITS: metal CONTRIBUTOR: MyACE1.jl - https://github.com/MyACEsuit/MyACE1.jl")
    push!(lines, "# MyACE1 pair potential")
    push!(lines, "")
...
function export_MyACE_functions(V3, species, reversed_species_dict)
    functions = Dict()
    lmax = 0
