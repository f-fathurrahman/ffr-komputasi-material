...
def poisson(F, mu, N, h, eps, r_add = 4, solver='Fourier'):   # check N+1
    ...
    if solver=='Fourier':
        U1_L[:, 0] = 4. * np.sin(1./(N+1)*(np.array(list(range(F.n[0])))+1)*pi/2)**2 / h**2
    elif solver=='spectral':
        U1_L[:, 0] = np.pi**2 * (np.array(list(range(F.n[0])))+1)**2
    else:
    ...
    frac = tuck.cross.multifun([f, L], eps, lambda a_b: a_b[0]/a_b[1], r_add = r_add)
    sol = tuck.dst(frac)
