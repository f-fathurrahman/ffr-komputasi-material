...
def exchange(psi1, psi2, i, eps_exchange, T, molecule):
    ...
    for j in range(molecule.orbitals):
        conv = tuck.cross.multifun([psi1[i], psi2[j]], eps_exchange, lambda a_b: a_b[0]*a_b[1], y0 = psi1[i])
        conv = tuck.cross.conv(T, conv, eps_exchange, y0 = conv)
        conv = tuck.round(tuck.real(conv),eps_exchange)
        conv = tuck.cross.multifun([psi2[j], conv], eps_exchange, lambda a_b1: a_b1[0]*a_b1[1],y0 = psi2[j])
