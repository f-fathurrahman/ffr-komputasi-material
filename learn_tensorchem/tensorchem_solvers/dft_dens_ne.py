...
def lda(molecule, psi, E, density, grid, T, eps, pot_coulomb, max_iter = 3, pr = None):
    ...
    for k in range(max_iter):
        ...
        for i in range(Norb):
            Vxc = tuck.cross.multifun([density, psi[i]], 10*eps, lambda a_b: ((cpot_fun(np.abs(np.real(a_b[0]))) + xpot_fun(np.abs(np.real(a_b[0]))))*(a_b[1])), y0 = psi[i], pr = None)
        ...
        for i in range(Norb):
            Vxc_new = tuck.cross.multifun([density, psi_Q[i]], 10*eps, lambda a_b1: ((cpot_fun(np.abs(np.real(a_b1[0]))) + xpot_fun(np.abs(np.real(a_b1[0]))))*(a_b1[1])), y0 = psi_Q[i], pr = None)
...
def lda_full_energy(molecule, psi, E, density, grid, T, eps, pr = None):
    ...
    prod = lambda A,B: tuck.cross.multifun([A,B], eps, lambda a_b3: a_b3[0]*a_b3[1])
    ...
    E_electr = 0
    e_xc = tuck.cross.multifun([density, density], eps, lambda a_b4: ((cen_fun(np.abs(np.real(a_b4[0]))) + xen_fun(np.abs(np.real(a_b4[0]))))*np.abs(a_b4[1])), y0 = density, pr = pr)
    de_xc = tuck.cross.multifun([density, density],  eps, lambda a_b5: ((cpot_fun(np.abs(np.real(a_b5[0]))) + xpot_fun(np.abs(np.real(a_b5[0]))))*(a_b5[1])), y0 = density, pr=pr)
...
def xen_low_fun(rho, xxx_todo_changeme6 = (gamma, beta1, beta2)):
    (gamma, beta1, beta2) = xxx_todo_changeme6
    rs = (3./(4*pi*rho))**(1./3) #rho = 1/(4/3 * pi * rs^3)  1/rho = 4/3 pi R^3 R = (3/(4*pi*rho))^(1/3)
    return gamma/(1. + beta1 * np.sqrt(rs) + beta2 * rs)

def xpot_high_fun(rho, xxx_todo_changeme7 = (A,B,C,D)):
    (A,B,C,D) = xxx_todo_changeme7
    rs = (3./(4*pi*rho))**(1./3)
    return A*np.log(rs) + (B-A/3) + 2./3*C*rs*np.log(rs) + 1./3*(2*D-C)*rs

def xpot_low_fun(rho, xxx_todo_changeme8 = (gamma, beta1, beta2)):
    (gamma, beta1, beta2) = xxx_todo_changeme8
    rs = (3./(4*pi*rho))**(1./3)
