...
E0_exact = dim*pi**2*D # 14.80440660, hbar^2/(2m)*pi^2/L^2* dim  
ET = 10.0 # a guess
kappa = 10.0
...
densfile =  'dens_particle_in_a_box_no_importance_sampling_tau='+str(tau)+'.dat'
print('saving density file ',densfile)

xw = np.zeros((Nwx,dim))
xw_tmp = np.zeros((Nwx,dim))       
alive = range(Nw)
...
# ====================
# walkers scattered randomly in the box 
#xw[alive] = rng.uniform(size=(Nw,dim))
xw[alive] = rng.uniform(low=0.1, high=0.9, size=(Nw,dim))
#xw[alive] = 0.5
...
Efile = 'E_particle_in_a_box_no_importance_sampling.dat'
