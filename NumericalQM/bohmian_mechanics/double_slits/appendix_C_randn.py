...
def rk4(wave, del_wave, wave2, del_wave2, x, h, y, f):
    k1 = h * f(wave, del_wave, wave2, del_wave2, x, y)
    k2 = h * f(wave, del_wave, wave2, del_wave2, x + 0.5*h, y + 0.5*k1)
    k3 = h * f(wave, del_wave, wave2, del_wave2, x + 0.5*h, y + 0.5*k2)
    k4 = h * f(wave, del_wave, wave2, del_wave2, x + h, y + k3)
    return x + h, y + (k1 + 2*(k2 + k3) + k4)/6.0

def trajectories(wave, del_wave, wave2, del_wave2, t, state):
    y, vel = state
...
PSI2 = PSI**2
wave2 = sympy.lambdify([y,t], PSI2, "numpy")
del_wave2 = sympy.lambdify([y,t], PSI2.diff(y), "numpy")

wave = sympy.lambdify([y,t], PSI,"numpy")
del_wave_x = sympy.lambdify([y,t], PSI.diff(y),"numpy")
...
for index,y0 in enumerate(inits):
    ...
    state = np.array([y0,0])
    while t < tf:
        t, state = rk4(wave, del_wave, wave2, del_wave2, t, dt, state, trajectories)
        x.append(t)
        y.append(state[0])
    ...
    filename = "TEMP_randn_-" + format("%05d" % index) + ".png"
