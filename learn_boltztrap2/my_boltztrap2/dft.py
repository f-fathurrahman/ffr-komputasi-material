...
import my_boltztrap2.io
import my_boltztrap2.misc
...
# way to add support for a new format in my_boltztrap2.
...
class VASPLoader:
    ...
    def __init__(self, directory):
        ...
        with my_boltztrap2.misc.dir_context(directory):
            vasprunxml = "vasprun.xml"
            if not os.path.isfile(vasprunxml):
                raise LoaderError("vasprun.xml not found")
            r = my_boltztrap2.io.parse_vasprunxml(vasprunxml)
            self.sysname = r["name"]
            self.atoms = r["atoms"]
            my_boltztrap2.misc.info("lattice:")
            my_boltztrap2.misc.info(self.atoms.get_cell().T)
...
class GenericWien2kLoader:
    ...
    def __init__(
        ...
        my_boltztrap2.misc.info("Wien2k system name:", w2kname)
        ...
        my_boltztrap2.misc.info("matching energy and scf files were found")
        ...
        my_boltztrap2.misc.info("lattice:", self.atoms.get_cell().T)
        my_boltztrap2.misc.info("conv:", conv)
        my_boltztrap2.misc.info("conv:", latt)
        self.kpoints, self.ebands = my_boltztrap2.io.W2Kene(energyfn, conv)
        self.fermi = my_boltztrap2.io.W2Kfermi(scffn)
        if os.path.isfile(derfn):
            my_boltztrap2.misc.info("a matching .mommat2 file was found")
            self.mommat, nemin, nemax = my_boltztrap2.io.W2Kmommat(
                derfn, self.kpoints
            )
...
def _get_W2Ksystemname(dirname):
    ...
    with my_boltztrap2.misc.dir_context(dirname):
        filenames = sorted(
...
class GENELoader:
    ...
    def __init__(self, directory):
        ...
        with my_boltztrap2.misc.dir_context(directory):
            my_boltztrap2.misc.info("GENE system name:", genename)
            if not os.path.isfile(energyfn):
                raise ValueError("energy file not found")
            self.atoms = my_boltztrap2.io.read_GENE_struct(structfn)
            my_boltztrap2.misc.info("lattice:", self.atoms.get_cell().T)
            (
                self.fermi,
                self.dosweight,
                ...
                self.ebands,
                mommat,
            ) = my_boltztrap2.io.read_GENE_eneandmat(energyfn)
...
class CASTEPLoader:
    ...
    def __init__(self, directory):
        ...
        with my_boltztrap2.misc.dir_context(directory):
            my_boltztrap2.misc.info("CASTEP system name:", castepname)
            if not os.path.isfile(energyfn):
                raise ValueError("energy file not found")
            (
                self.atoms,
                magmom,
                fict_magmom,
            ) = my_boltztrap2.io.read_CASTEP_output(structfn)
            my_boltztrap2.misc.info("lattice:", self.atoms.get_cell().T)
            (
                self.fermi,
                ...
                self.nelect,
                mommat,
            ) = my_boltztrap2.io.read_CASTEP_bands(energyfn)
...
class ABINITLoader:
    ...
    def __init__(self, directory):
        ...
        with my_boltztrap2.misc.dir_context(directory):
            my_boltztrap2.misc.info("ABINIT GSR file name:", abinitname)
            r = my_boltztrap2.io.parse_abinitrun(abinitname)
            self.sysname = r["name"]
            self.atoms = r["atoms"]
            my_boltztrap2.misc.info("lattice:", self.atoms.get_cell().T)
            self.kpoints = r["kpoints"]
...
class AIMSLoader:
    ...
    def __init__(self, directory):
        ...
        with my_boltztrap2.misc.dir_context(directory):
            my_boltztrap2.misc.info("AIMS system name:", directory)
            ...
            aimsreader = my_boltztrap2.io.parse_aims(directory)
...
class ESPRESSOLoader:
    ...
    def __init__(self, directory):
        ...
        with my_boltztrap2.misc.dir_context(directory):
            r = my_boltztrap2.io.parse_ESPRESSO_xml(filename)
...
class GPAWLoader:
    ...
    def __init__(self, directory):
        ...
        (
            ...
            self.ebands,
            self.nelect,
        ) = my_boltztrap2.io.read_GPAW(gpaw_file)
...
class DFTData:
    ...
    def __init__(self, directory, derivatives=False, *args, **kwargs):
        ...
        for label, loader in loaders[::-1]:
            my_boltztrap2.misc.info("looking for a {} calculation".format(label))
            try:
                loaded = loader(directory, *args, **kwargs)
            except LoaderError as e:
                my_boltztrap2.misc.info("error in {} loader: {}".format(label, e))
                continue
            self.source = label
            break
        else:
        ...
        my_boltztrap2.misc.info("Fermi energy:", self.fermi)
        ...
        try:
            ...
            self.magmom = None
            my_boltztrap2.misc.info("Assuming a non-spin-polarized calculation")
        # If the number of valence electrons has not been set yet, compute it
        # from the bands.
        try:
            self.nelect = loaded.nelect
        except AttributeError:
            degeneracies = my_boltztrap2.sphere.calc_reciprocal_degeneracies(
    ...
    def bandana(self, emin=-np.inf, emax=np.inf):
        ...
        my_boltztrap2.misc.info("BANDANA output")
