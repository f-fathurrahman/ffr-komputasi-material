...
%bcc_dos.m
%Density of states for the single band, body centered cubic system
...
e2=1.0;
e1=-e2;
eminl=-0.05; eminr=-eminl;
e0a=e1:x:eminl; e0b=eminr:x:e2; %not to do in between eminl and eminr
e0=[e0a,e0b];                   %energy range, but skip small e range
ntmax=length(e0);
a=1.0;          %lattice constant
tpa=2*pi/a;
VBZ=2*tpa^3;    %total BCC BZ volume
%BCC case tetrahedron (only one needed) in units of 2*pi/a;
P=[1/2,1/2,1/2]; H=[1,0,0]; N=[1/2,1/2,0]; %tetrahedron symmetry points
%Tetrahedron q vectors according to the method of An-Ban Chen & B. I. Reser.
q(1,:)=P*tpa; q(2,:)=(H-P)*tpa ; q(3,:)=(N-H)*tpa;
%total tetrahedron volume
VBCC_t=abs(dot(P,cross(H,N))/6)*VBZ;
lim=9; lom=lim-1;              %lim must be odd
%number of divisions along q2, and q3 => total number of TT's is n^2
n=25;
%VBCC_t is the standard tetrahedron volume part, and there are 24 of them in
%the body center cube's total BZ
factor=VBCC_t*24;    %initital factor used in the full integral result
...
%Notation: va(ith TT vector,coordinate(x,y,z))
...
for nt=1:ntmax
  ...
  for it=1:Ntt       %Ntt TT's
    for ko=1:lim   %loop over alpha
      ...
      top(ko)=al(ko)*al(ko);
      deno(ko)=e0(nt)+cos(k(1)/2)*cos(k(2)/2)*cos(k(3)/2)+im*delta;
    end
    gy=singInt(top,deno,dk);
    dos(nt)=-imag(gy)/pi+dos(nt);  %single TT dos, add all n^2 TT's
  end
  dos(nt)=factor*dos(nt)/VBZ;
  ge(nt)=jelittoBccDos(e0(nt))/VBZ;
...
fprintf('Integrated Density of States for the body centered cubic')
...
str=cat(2,'D(E) and N(E) for the body centered cubic (no spin)');
title(str, 'Fontsize',12)
legend('Exact D(E)','numeric D(E)','N(E)',0)

function ge=jelittoBccDos(ee)
%Jelitto's DOS for the body centered cubic (exact)
eaa=abs(ee);
if (eaa < 1.)
    ge=2.0*sqrt(1.-eaa)*(log(5.845/eaa))^2;
    ge=ge*(16.6791+3.6364*eaa+2.4880*eaa^2);
else
    ge=0.0;
end
