...
using MyACE1.RPI: get_picoeffs
using MyACE1.DAG: CorrEvalGraph
...
# ------------------------------------------------------------
#   Total Energy
#   this is basically a copy-paste of the MyJuLIP implementation
#   surely we can do this more generally ... 

using Base.Threads: nthreads, threadid, @threads
using MyJuLIP: neighbourlist, maxneigs, AbstractCalculator
using MyJuLIP.Potentials: neigsz!, site_virial
...
using MyACE1.PairPotentials: PolyPairBasis, PolyPairPot
...
function _dot_zij_co(V, B, z, z0)
   i0 = MyACE1.PairPotentials._Bidx0(V.basis, z, z0)  # cf. pair_basis.jl
...
function co_evaluate!(tmp, V::PolyPairPot, r::Number, z, z0) 
   ...
   v = MyACE1.PairPotentials._dot_zij(V, tmp.J[Iz, Iz0], z, z0)
...
function co_evaluate_d!(tmp, V::PolyPairPot, r::Number, z, z0) 
   ...
   dv = MyACE1.PairPotentials._dot_zij(V, tmp.dJ[Iz, Iz0], z, z0)
...
import MyJuLIP.MLIPs: IPSuperBasis
...
function committee_potential(basis::IPSuperBasis, 
   ...
   return MyJuLIP.MLIPs.SumIP(co_pots)
end
