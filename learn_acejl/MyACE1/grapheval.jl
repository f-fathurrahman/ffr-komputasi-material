...
==(dag1::CorrEvalGraph, dag2::CorrEvalGraph) = MyACE1.allfieldsequal(dag1, dag2)
...
write_dict(gr::CorrEvalGraph{T, TI}) where {T <: Real, TI <: Integer} =
   Dict( "__id__" => "MyACE1_CorrEvalGraph",
         "T" => write_dict(T),
         "TI" => write_dict(TI),
...
function read_dict(::Val{:MyACE1_CorrEvalGraph}, D::Dict)
   T = read_dict(D["T"])
   TI = read_dict(D["TI"])
   @assert T <: Real
   @assert TI <: Integer
