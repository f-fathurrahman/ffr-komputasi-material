...
import MyACE1
import MyACE1.RPI: BasicPSH1pBasis, SparsePSHDegree, RPIBasis, get_maxn, 
                 SparsePSHDegreeM
import MyACE1: PolyTransform, transformed_jacobi
...
function splinify(basis::RPIBasis)
   ...
   J_spl = MyACE1.Splines.RadialSplines(basis1p.J; zlist = basis1p.zlist)
   basis1p_spl = MyACE1.RPI.BasicPSH1pBasis(J_spl, deepcopy(basis1p.SH), 
                  deepcopy(basis1p.zlist), deepcopy(basis1p.spec), deepcopy(basis1p.Aindices) )
   pibasis_spl = MyACE1.PIBasis(basis1p_spl, deepcopy(basis1p.zlist), 
