...
using MyACE1.SphericalHarmonics: SHBasis, index_y
using StaticArrays
using MyJuLIP: AbstractCalculator, Atoms, JVec
using MyJuLIP.Potentials: SitePotential, SZList, ZList
using NeighbourLists: neigs

import MyJuLIP, MyJuLIP.MLIPs
...
function deletezeros(V::PIPotential)
   ...
   for iz0 = 1:length(V.coeffs)
      ...
      b2iAA = Dict{MyACE1.PIBasisFcn, Int}()   # new
      ...
      push!(innernew,
            MyACE1.InnerPIBasis( inner.orders[Inz],
                              iAA2iA,
                              b2iAA,
                              inner.b2iA,
                              (idx0+1):(idx0+length(Inz)),
                              inner.z0,
                              MyACE1.DAG.CorrEvalGraph{Int, Int}() ) )
...
write_dict(V::PIPotential) = Dict(
        "__id__" => "MyACE1_PIPotential",
        "meta" => Dict("MyACE1_version" => pkgversion(MyACE1)),
       "pibasis" => write_dict(V.pibasis),
        "coeffs" => [ write_dict.(V.coeffs)... ], 
     "committee" => write_committee(V.committee) )

#    if ntests > 0
#       tests = MyACE1.Random.rand_nhd(Nat, J::ScalarBasis, species = :X)
#       Pr = V.pibasis.basis1p.J
#       for ntest = 1:5
#
#          r = MyACE1.rand_radial(V.pibasis.basis1p.J)
#          Pr = evaluate(B3.J, r)
...
read_dict(::Val{:MyACE1_PIPotential}, D::Dict; tests = true)  = 
