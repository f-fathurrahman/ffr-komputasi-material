...
import MyJuLIP: z2i, AtomicNumber 
import Interpolations, MyACE1, ForwardDiff

using Interpolations: cubic_spline_interpolation, Flat, OnGrid

using MyJuLIP.Potentials: z2i, i2z, SZList
import MyJuLIP.Potentials: cutoff 
import MyACE1: alloc_B, alloc_dB, alloc_temp, alloc_temp_d, 
...
struct RadialSplines{T, NZ, SPLINE} <: MyACE1.ScalarBasis{T}
...
function RadialSplines_hermite(J; nnodes = 1_000, zlist = J.trans.zlist)
   ...
   for iz = 1:NZ, iz0 = 1:NZ
      ...
      for (ir, r) in enumerate(rr) 
         ...
         dJ_zz0[:, ir] = MyACE1.evaluate_d(J, r, z, z0)
      end
      for ib = 1:NB 
...
function cutoff(J::MyACE1.OrthPolys.TransformedPolys, z::AtomicNumber, z0::AtomicNumber)
   if !(J.trans isa MyACE1.Transforms.MultiTransform)
      return cutoff(J) 
   end 

   if !( (J.J.tl ≈ -1) && (J.J.tr ≈ 1) && 
         all(trans isa MyACE1.Transforms.AffineT 
   ...
   rcut = try
      rr = MyACE1.Transforms.inv_transform(J.trans, 1.0, z, z0)
      rl = MyACE1.Transforms.inv_transform(J.trans, -1.0, z, z0)
      max(rr, rl)
   catch 
...
function write_dict(basis::RadialSplines{T})  where {T} 
   ...
   return Dict("__id__" => "MyACE1_RadialSplines",
                "T" => write_dict(T), 
...
function read_dict(::Val{:MyACE1_RadialSplines}, D::Dict)
   T = read_dict(D["T"])
