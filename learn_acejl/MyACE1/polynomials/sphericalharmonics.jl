...
import MyJuLIP.MLIPs: IPBasis, alloc_B, alloc_dB, fltype
import MyJuLIP: alloc_temp, alloc_temp_d, evaluate!, evaluate_d!,
...
write_dict(SH::SHBasis{T}) where {T} =
		Dict("__id__" => "MyACE1_SHBasis",
			  "T" => write_dict(T),
			  "maxL" => SH.maxL)

read_dict(::Val{:MyACE1_SHBasis}, D::Dict) =
