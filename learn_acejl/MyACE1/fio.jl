...
function to load an MyACE1 potential. If `format == :auto` it tries to 
...
* `format = :zip` for a zipped json file that was written using `MyJuLIP.FIO.zip_dict`
"""
...
function to save an MyACE1 potential to disk. If `format == :auto` it tries to 
guess the format from the ending of the filename. If this fails, then 
one can try to pass the format explicitly as a kwarg, 
* `format = :json` for a raw json file 
* `format = :zip` for to write a compressed json file using `MyJuLIP.FIO.zip_dict`
"""
function save_potential(fname::AbstractString, V; format = :auto)
   ...
   D = write_dict(V)
   if format == :json 
      MyJuLIP.FIO.save_dict(fname, D)
   elseif format == :zip 
      MyJuLIP.FIO.zip_dict(fname, D)
   else      
      error("Unknown file format")
   end
end
