...
export PolyPairPot
import MyACE1: write_committee, read_committee
...
MyJuLIP.MLIPs.combine(pB::PolyPairBasis, coeffs::AbstractVector) =
...
MyJuLIP.cutoff(V::PolyPairPot) = cutoff(V.basis)
...
read_dict(::Val{:MyACE1_PolyPairPot}, D::Dict, T = read_dict(D["T"])) =
      PolyPairPot(read_dict(D["basis"]), T.(D["coeffs"]), 
                  read_committee(D["committee"]))

import MyACE1
MyACE1.ncommittee(V::PolyPairPot{T, TJ, NZ, NCO}) where {T,TJ,NZ, NCO} = 
