...
function neigsz!(tmp, nlist::PairList, at::MyACE1.Atoms, i::Integer)
    # from JuLIP
...
are defined in `MyACEmd.default_energy` and `MyACEmd.default_length`.
...
function load_ace_model(fname::AbstractString;
    ...
    if old_format
        ...
        return MyACEpotential(pot.components; energy_unit=energy_unit, length_unit=length_unit, cutoff_unit=cutoff_unit)
    end
end
...
function get_cutoff(V; cutoff_unit=default_length)
    return MyACE1.cutoff(V) * cutoff_unit
end

function get_cutoff(V::MyACEpotential; kwargs...)
    c = map( V ) do v
        if typeof(v) <: MyACE1.OneBody
            return 0
        else
            return MyACE1.cutoff(v)
        end
    end
...
function neighborlist(at::MyACE1.Atoms, cutoff; kwargs...)
    return MyACE1.neighbourlist(at, ustrip(u"Å", cutoff); kwargs...)
end



_atomic_number(at::MyACE1.Atoms, i)          = at.Z[i]
_atomic_number(ab::AbstractSystem, i) = MyACE1.AtomicNumber(AtomsBase.atomic_number(ab,i))