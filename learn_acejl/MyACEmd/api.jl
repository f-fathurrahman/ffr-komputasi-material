...
Calculates MyACE potential energy for atomic system.
...
function ace_energy(V::MyACE1.OneBody, at::MyACE1.Atoms; domain=1:length(at), energy_unit=default_energy, kwargs...)
    E = sum( domain ) do i
        MyACE1.evaluate(V, MyACE1.chemical_symbol(at.Z[i]) )
    end
    return E * energy_unit
end

function ace_energy(V::MyACE1.OneBody, as::AbstractSystem; domain=1:length(as), energy_unit=default_energy, kwargs...)
    E = sum( domain ) do i
        MyACE1.evaluate(V, atomic_symbol(as, i) )
    end
    return E * energy_unit
end
...
# Generate interface for MyACEpotential type
for ace_method in [ :ace_energy, :ace_forces, :ace_virial, :ace_atom_energies ]
    @eval begin
        function $ace_method(calc::MyACEpotential, at;
                domain=1:length(at),
                executor=ThreadedEx(),
                ntasks=Threads.nthreads(),
...
Calculates forces for MyACE potential for given atomic system.
The `atoms` object needs to be in `AtomsBase` compatable format.
The returned energy has a unit as defined by `Unitful`.
...
function ace_forces(::MyACE1.OneBody, at::MyACE1.Atoms; energy_unit=default_energy, length_unit=default_length, kwargs...)
    T = (eltype ∘ eltype)(at.X)
    F = [ SVector{3}( zeros(T, 3) ) * (energy_unit / length_unit) for _ in 1:length(at) ]
    return F
end

function ace_forces(::MyACE1.OneBody, as::AbstractSystem; energy_unit=default_energy, length_unit=default_length, kwargs...)
...
Calculates virial for MyACE potential for given atomic system.
The `atoms` object needs to be in `AtomsBase` compatable format.
The returned energy has a unit as defined by `Unitful`.
...
function ace_virial(::MyACE1.OneBody, at::MyACE1.Atoms; energy_unit=default_energy, length_unit=default_length, kwargs...)
    T = (eltype ∘ eltype)(at.X)
    return SMatrix{3,3}(zeros(T, 3,3)) * energy_unit
end

function ace_virial(::MyACE1.OneBody, as::AbstractSystem; energy_unit=default_energy, length_unit=default_length, kwargs...)
...
Calculates MyACE potential energy for each atom.
The `atoms` object needs to be in `AtomsBase` compatable format.
The returned energy has a unit as defined by `Unitful`.
...
function ace_atom_energies(V::MyACE1.OneBody, at::MyACE1.Atoms; domain=1:length(at), energy_unit=default_energy, kwargs...)
    E = map( domain ) do i
        MyACE1.evaluate(V, MyACE1.chemical_symbol(at.Z[i]) )
    end
    return E * energy_unit
...
function ace_atom_energies(V::MyACE1.OneBody, as::AbstractSystem; domain=1:length(as), energy_unit=default_energy, kwargs...)
    E = map( domain ) do i
        MyACE1.evaluate(V, atomic_symbol(as, i) )
    end
    return E * energy_unit
...
function ace_energy(basis::MyACE1.IPBasis, at; domain=1:length(at), cutoff_unit=default_length, executor=ThreadedEx(), kwargs...)
...
function ace_forces(
    pair_basis::MyACE1.PolyPairBasis,
    at;
    domain=1:length(at),
    executor=ThreadedEx(),
    ntasks=Threads.nthreads(),
...
function ace_forces(
    ...
    tmp = MyACE1.alloc_temp_d(pair_basis)
    for I in domain
        ...
        Ii = MyACE1.z2i(pair_basis, Zi)
        Ij = MyACE1.z2i(pair_basis, Zj)
        dJ = tmp.dJ[Ii, Ij]
        MyACE1.evaluate_d!(tmp.J[Ii, Ij], dJ, tmp.tmpd_J[Ii, Ij], pair_basis.J[Ii, Ij], r, Zi, Zj)
...
for ace_method in [ :ace_energy, :ace_forces, :ace_virial]
    @eval begin
        function $ace_method(basis::MyACE1.IPSuperBasis, data; kwargs...)
...
function ace_forces_virial(basis::MyACE1.IPSuperBasis, data; kwargs...)
    em = map( basis.BB ) do b
