...
using MyJuLIP: Atoms, AbstractAtoms, AbstractCalculator,
      JVec, mat, vec, JMat, SVec, vecs, SMat,
      positions, set_positions!, fltype_intersect

using MyJuLIP.Chemistry: atomic_number
...
import MyJuLIP: energy, forces, cutoff, virial, hessian_pos, hessian,
              site_energies, r_sum,
...
function energy(V::SitePotential, at::AbstractAtoms; kwargs...) 
   tmp = [alloc_temp(V, at) for i in 1:MyJuLIP.nthreads()]
...
function virial(V::SitePotential, at::AbstractAtoms; kwargs...) 
   tmp = [alloc_temp_d(V, at) for i in 1:MyJuLIP.nthreads()]
