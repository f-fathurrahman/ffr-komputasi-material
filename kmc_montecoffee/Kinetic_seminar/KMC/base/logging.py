...
class Log:
    ...
    def __init__(self, parameters, accelparams = {"on": False}):
        ...
        self.f.write(r'''  __  __             _        ____       __  __           
...
''')
        self.f.write('MonteCoffee is developed at Chalmers University of Technology with contributions from:\n')
        ...
        for p in parameters.keys():
            self.f.write(format(str(p),'<10')+format(':','<5')+
                    str(parameters[p])+'\n')

        self.f.write('\n'+'-'*80+'\n'*3)
        self.f.write('kinetic Monte Carlo Log \n\n')
        self.f.write('{:<10s} {:^20s} {:^20s} {:^30s} {:<10s}'.format('Step',
                'time[hr:min:s]','Sim time [s]','Sim T [K]',str(parameters["Events"])+' \n'))
    ...
    def dump_point(self, step, sim_time, sim_T, ev_called):
...
#        with open(self.fn, 'a') as f:
        ...
        self.f.write('{:<10s} {:^20s} {:^20s} {:^30s} {:<10s}'.format(str(step),
                    time_str,str(sim_time),str(sim_T), str(["%.0f"%item for item in ev_called])+'\n'))
...
#        atoms_write=copy.deepcopy(sim.system.surf_atoms)
        acell = sim.system.atoms.get_cell()
...
#        sym_ext = list(atoms_write.get_chemical_symbols())
        ...
        for i in range(sim.system.Nsites):
          if sim.system.sites[i].stype == 0:
              pos.append([pos_ind[sim.system.sites[i].ind][0], pos_ind[sim.system.sites[i].ind][1],pos_ind[sim.system.sites[i].ind][2]])
              syms.append('Pt')
          if sim.system.sites[i].covered == 1:
            syms.append('C')
            pos.append([pos_ind[sim.system.sites[i].ind][0],pos_ind[sim.system.sites[i].ind][1],max_z+1.5])
